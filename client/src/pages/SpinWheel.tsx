...
function SpinWheelCanvas({ spinning, targetIndex, onAnimationEnd }: {
  spinning: boolean;
  targetIndex: number;
  onAnimationEnd: () => void;
}) {
  ...
  const drawWheel = (rotation: number) => {
    ...
    SEGMENTS.forEach((seg, i) => {
      ...
      ctx.font = `bold ${canvas.width < 280 ? 9 : 11}px Inter, sans-serif`;
  ...
  useEffect(() => {
    ...
    const segCount = SEGMENTS.length;
    const segAngle = (2 * Math.PI) / segCount;
    // Calculate target rotation so the pointer (top) lands on targetIndex
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center p-8">
          <h2 className="text-xl font-bold text-[#1B2A5E] mb-4">Sign in to spin the wheel</h2>
      ...
      <div className="md:ml-56">
        <div className="prime-gradient text-white px-4 pt-6 pb-12">
          <div className="container max-w-lg text-center">
            ...
            <p className="text-white/70 text-sm">One free spin every day — win points, discounts, and more!</p>
          </div>
        </div>
        ...
        <div className="container max-w-lg -mt-8 pb-6">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 text-center">
            ...
            <div className="flex items-center justify-center gap-2 mb-6">
              ...
              <span className="text-sm font-semibold text-gray-600">{(customer?.totalPoints ?? 0).toLocaleString()} points</span>
            </div>
            ...
            {canSpin ? (
              ...
              <div className="flex flex-col items-center gap-2">
                <div className="flex items-center gap-2 text-gray-400 text-sm">
                  <Clock size={16} />
                  <span>You've used your spin for today</span>
                </div>
                <p className="text-xs text-gray-400">Come back tomorrow for another spin!</p>
              </div>
            )}
          </div>
          ...
          <div className="mt-4 bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            <h3 className="font-semibold text-[#1B2A5E] mb-3">Possible Prizes</h3>
            <div className="grid grid-cols-3 gap-2">
              {[
                { icon: Star, label: "50 Points", color: "#1B2A5E" },
                { icon: Star, label: "100 Points", color: "#2d4a9e" },
                { icon: Star, label: "200 Points", color: "#6366F1" },
                { icon: Gift, label: "10% Discount", color: "#5B9BD5" },
                { icon: Gift, label: "20% Discount", color: "#F59E0B" },
                { icon: Truck, label: "Free Delivery", color: "#10B981" },
                { icon: Palette, label: "Free Design", color: "#EC4899" },
                { icon: Zap, label: "2x Points", color: "#F97316" },
        ...
        {showResult && result && (
          ...
          >
            ...
            >
              ...
              <h2 className="text-2xl font-bold text-[#1B2A5E] mb-2">You Won!</h2>
