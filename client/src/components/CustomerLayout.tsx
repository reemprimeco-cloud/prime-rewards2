...
const navItems = [
  ...
  { href: "/spin", label: "Spin & Win", icon: RotateCcw },
...
const extraNavItems = [
  ...
  { href: "/transactions", label: "History", icon: FileText },
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center p-8">
          ...
          <h2 className="text-xl font-bold text-prime-navy mb-2">Sign in to continue</h2>
          <p className="text-gray-500 mb-6">Access your Prime Rewards account</p>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-40 bg-black/50 md:hidden" onClick={() => setMenuOpen(false)}>
          <div className="absolute right-0 top-14 w-72 bg-white h-full shadow-xl" onClick={e => e.stopPropagation()}>
      ...
      {/* Desktop Sidebar */}
      <div className="hidden md:block fixed left-0 top-14 h-[calc(100vh-3.5rem)] w-56 bg-[#1B2A5E] text-white z-20 overflow-y-auto flex flex-col">
        <nav className="p-4 space-y-1">
          {navItems.map(({ href, label, icon: Icon }) => {
            const active = location === href;
            return (
